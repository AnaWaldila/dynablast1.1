classdef Class_TNT
    ...
    % parameter_dist: input data of blast main equation (1 for using scaled
    % distance Z or 2 for using R)
    % R: distance (m)
    ...
    methods
        ...
        function tnt = Class_TNT(parameter_dist, R, Z, W, type, negative, ...
                db_pmax, db_pmin, db_td, db_id, db_im, option)
            if (nargin > 0)

                tnt.parameter_dist  = parameter_dist;
                tnt.R               = R;
                tnt.Z               = Z;
                tnt.W               = W;
                tnt.type            = type;
